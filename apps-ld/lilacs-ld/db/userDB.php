...
*/
//api_key no permiido en get ni update
...
function getUsersDB($params, $q='', $fields='', $sort_by='', $sort_order='', $page_size=10, $page=1){       
  ...
  //total
  $total = getCount($link, 'user', $where);
  if($total){  
    if($where){
      $where = "WHERE ". $where;  
    }
  ...
  if(!$data_arr OR !isset($data_arr['email']) /*OR !isset($data_arr['api_key'])*/) {
    ...
    if($u){//ya existe o error
      ...
      if(isset($data_arr['permisos'])){
        $perm_arr = validateJson($data_arr['permisos'], $link);
        if(!$perm_arr) {
      ...
      $permisos_str = array_to_objStr($perm, $link);
    
      $r = mysqli_query($link, "INSERT INTO user (email, api_key, permisos, created) VALUES ('".$data_arr['email']."', '".$api_key."', JSON_OBJECT($permisos_str), NOW())");
...
function updateUserDB($id, $data){
  global $link, $allowed_ufields, $entidades;
      ...
      switch($key){
        ...
        case 'permisos':
          if(is_array($value)){
            ...
            if($perm){
              $perm_str = array_to_objStr($perm, $link,0,1);  
              $set[] = "permisos=JSON_SET(permisos,$perm_str)";
            }
            else
              $err_msg .= "Error en parametro: 'permisos' no tiene un valor valido. ";
          }  
          else
            $err_msg .= "Error en parametro: 'permisos' no tiene un valor valido. ";
          break;  
...
function deleteUserDB($id){
  ...
  $r = mysqli_query($link, "DELETE FROM user WHERE id = $id");
  return array('data'=>$r);
}

function preparePerm($value){
  $entidades = array('documents', 'persons', 'organizations', 'events', 'projects', 'users');
